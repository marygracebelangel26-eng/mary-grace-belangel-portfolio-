...
export default function Header({ activeView, setActiveView }: HeaderProps) {
  ...
  return (
    ...
    >
      {/* Full-Screen Backdrop Blur Overlay to blur the background content when menu is open */}
      <div
        className={`fixed inset-0 bg-black/90 backdrop-blur-2xl z-20 transition-all duration-300 lg:hidden ${
      ...
      <div className="relative z-50 max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2 sm:space-x-6 lg:space-x-12">
          ...
          <button
            ...
            className="lg:hidden p-2 text-[#F5F5F5] hover:text-[#14B8A6] focus:outline-none -ml-2 cursor-pointer relative z-50"
      ...
      <div
        className={`fixed inset-x-0 bottom-0 ${
          scrolled ? 'top-[60px]' : 'top-[76px]'
        } z-30 bg-[#0A0A0A]/98 backdrop-blur-3xl border-t border-[#262626] transition-all duration-300 lg:hidden flex flex-col justify-between p-6 ${
          ...
          {navLinks.map((link, idx) => {
            ...
            return (
              <a
                ...
                href={link.href}
                onClick={(e) => handleNavLinkClick(e, link.viewId)}
                className={`text-base font-medium tracking-wide transition-all py-1.5 px-3 rounded-md flex items-center justify-between group focus:outline-none ${
                  active ? 'text-[#14B8A6] bg-[#14B8A6]/10 font-semibold' : 'text-[#F5F5F5] hover:text-[#14B8A6] hover:bg-white/5'
                }`}
                style={{ transitionDelay: `${idx * 40}ms` }}
              >
                <span>{link.name}</span>
                <span className={`w-1.5 h-1.5 rounded-full bg-[#14B8A6] transition-opacity ${
